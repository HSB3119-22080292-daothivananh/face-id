...
function StepCCCD({
  ...
  const handleFront = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (response.success && response.data && Object.keys(response.data).length > 0) {
        // Mặt trước: điền toàn bộ dữ liệu từ YOLO
  ...
  // ── Mặt sau: CHỈ điền 3 field: issue_date, issued_by, special_features ───
  const handleBack = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // ✅ Chỉ cập nhật đúng 3 field từ mặt sau, KHÔNG đụng vào data mặt trước
        setCccd((prev) => ({
          ...prev,
  ...
  const StatusBadge = ({
    status, scanning, color,
  }: { status: string; scanning: boolean; color: string }) => (
    <div style={{
      position: "absolute", top: 4, right: 4,
      width: 18, height: 18, borderRadius: "50%",
      background:
        status === "ok"   ? "#00ff88" :
        status === "fail" ? "#ffc800" : color,
      display: "flex", alignItems: "center", justifyContent: "center",
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
        ...
        <div>
          ...
          >
            <input
              ref={frontRef} type="file" accept="image/jpeg, image/png, image/webp"
              style={{ display: "none" }} onChange={handleFront}
        ...
        <div>
          ...
          >
            <input
              ref={backRef} type="file" accept="image/jpeg, image/png, image/webp"
              style={{ display: "none" }} onChange={handleBack}
              onClick={(e) => e.stopPropagation()}
            />
            {backImg ? (
              <>
      ...
      {(frontStatus === "fail" || backStatus === "fail") && (
        <motion.div
          ...
          style={{
            marginBottom: 12, padding: "8px 12px", borderRadius: "8px",
            background: "rgba(255,200,0,0.06)", border: "1px solid rgba(255,200,0,0.2)",
            fontSize: "11px", color: "#ffc800", display: "flex", gap: 6, alignItems: "flex-start",
          }}
        >
      ...
      <div style={{
        background: "rgba(0,200,80,0.03)", border: "1px solid rgba(0,200,80,0.1)",
        borderRadius: "12px", padding: "14px", marginBottom: 16,
      }}>
      ...
      <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
          {[
            ...
            <motion.div
              key={m.id} whileHover={{ x: 4 }} whileTap={{ scale: 0.98 }}
              onClick={() => setMode(m.id as "webcam" | "upload")}
              style={{
                display: "flex", alignItems: "center", gap: 14,
                padding: "14px 16px", borderRadius: "14px",
                background: `${m.color}06`, border: `1px solid ${m.color}18`, cursor: "pointer",
              }}
            >
    ...
    <div>
      <div style={{ position: "relative", borderRadius: "16px", overflow: "hidden", background: "#020408", aspectRatio: "4/3", border: "2px solid rgba(0,212,255,0.2)", marginBottom: 12 }}>
        {mode === "webcam" ? (
          <>
            ...
            {!camActive ? (
              <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12 }}>
                {camError && <div style={{ color: "#ffc800", fontSize: "11px", textAlign: "center", padding: "0 20px" }}>{camError}</div>}
                <button onClick={startWebcam} style={{ padding: "12px 24px", borderRadius: "12px", background: "#00d4ff", border: "none", color: "#000", fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <>
                ...
                <div style={{ position: "absolute", bottom: 14, left: 0, right: 0, display: "flex", justifyContent: "center", gap: 10 }}>
                  <button onClick={snapPhoto} disabled={files.length >= 5} style={{ padding: "10px 24px", borderRadius: "20px", background: files.length < 5 ? "#00ff88" : "#333", border: "none", fontWeight: 700, cursor: files.length < 5 ? "pointer" : "not-allowed" }}>
                    📸 Chụp ({files.length}/5)
                  </button>
                  <button onClick={stopWebcam} style={{ padding: "10px 16px", borderRadius: "20px", background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.15)", color: "#e2e8f0", cursor: "pointer" }}>
                    Dừng
                  </button>
                </div>
              </>
          ...
          <div onClick={() => fileRef.current?.click()} style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", gap: 8 }}>
            <input ref={fileRef} type="file" multiple accept="image/jpeg, image/png, image/webp" onChange={handleUpload} onClick={(e) => e.stopPropagation()} style={{ display: "none" }} />
      ...
      {previews.length > 0 && (
        <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
          {previews.map((src, i) => (
            <motion.div key={i} initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} style={{ position: "relative", width: 56, height: 56, borderRadius: "8px", overflow: "hidden", border: "1px solid #00d4ff" }}>
              <img src={src} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt={`face-${i}`} />
              <button onClick={() => removeFile(i)} style={{ position: "absolute", top: 2, right: 2, background: "rgba(0,0,0,0.7)", border: "none", cursor: "pointer", borderRadius: "50%", width: 16, height: 16, display: "flex", alignItems: "center", justifyContent: "center", padding: 0 }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <button onClick={() => { stopWebcam(); setMode("choose"); }} style={{ flex: 1, padding: "10px", borderRadius: "10px", background: "transparent", border: "1px solid rgba(255,255,255,0.08)", color: "#7a95b8", cursor: "pointer" }}>
          ← Chọn lại
        </button>
        <button onClick={() => onNext(files)} disabled={files.length === 0} style={{ flex: 2, padding: "10px", borderRadius: "10px", background: files.length > 0 ? "#00d4ff" : "#1a2a3a", border: "none", color: files.length > 0 ? "#000" : "#4a6fa5", fontWeight: 700, cursor: files.length > 0 ? "pointer" : "not-allowed" }}>
          Tiếp theo →
        </button>
      </div>
    </div>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 16 }}>
        {fields.map((f) => (
          <div key={f.label}>
            <label style={{ fontSize: "12px", color: "#4a6fa5", display: "block", marginBottom: 4 }}>{f.label}</label>
            <input
              type={f.type} value={f.value}
              onChange={(e) => f.setter(e.target.value)}
              placeholder={f.placeholder}
              style={{ ...inputStyle, borderColor: f.required && f.value ? "rgba(0,255,136,0.3)" : "rgba(0,212,255,0.15)" }}
            />
          </div>
        ))}
      </div>
      ...
      {error && (
        <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: 12, padding: "8px 12px", borderRadius: "8px", background: "rgba(255,107,107,0.08)", border: "1px solid rgba(255,107,107,0.3)", color: "#ff6b6b", fontSize: "12px", display: "flex", gap: 6, alignItems: "center" }}>
          <AlertTriangle size={12} /> {error}
        </motion.div>
      )}

      <div style={{ display: "flex", gap: 10 }}>
        <button onClick={onBack} disabled={submitting} style={{ flex: 1, padding: "11px", borderRadius: "10px", background: "transparent", border: "1px solid rgba(255,255,255,0.08)", color: "#7a95b8", cursor: submitting ? "not-allowed" : "pointer" }}>
          ← Quay lại
        </button>
        <button
...
function StepLoading() {
  ...
  return (
    <div style={{ textAlign: "center", padding: "50px 0" }}>
      ...
      <div style={{ color: "#4a6fa5", fontSize: "12px" }}>Vui lòng không đóng cửa sổ này</div>
      <div style={{ marginTop: 24, height: 4, borderRadius: 4, background: "rgba(255,255,255,0.05)", overflow: "hidden", width: "60%", margin: "24px auto 0" }}>
        <motion.div animate={{ x: ["-100%", "200%"] }} transition={{ duration: 1.2, repeat: Infinity, ease: "easeInOut" }} style={{ height: "100%", width: "50%", background: "linear-gradient(90deg, transparent, #00d4ff, transparent)", borderRadius: 4 }} />
      </div>
    </div>
  );
}
...
export function RegisterModal({ onClose, onSuccess }: Props) {
  ...
  return (
    <motion.div
      ...
      style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.6)", backdropFilter: "blur(15px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 300, padding: 16 }}
    >
      <motion.div
        initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
        style={{ width: "100%", maxWidth: 580, maxHeight: "95vh", overflowY: "auto", background: "var(--app-surface)", border: "1px solid rgba(0,212,255,0.2)", borderRadius: "24px", padding: "24px", scrollbarWidth: "none" }}
      >
        {/* Header */}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
          ...
          {!submitting && (
            <button onClick={handleClose} style={{ background: "var(--app-bg-subtle)", border: "1px solid var(--app-border)", borderRadius: "8px", cursor: "pointer", color: "var(--app-muted)", padding: "6px", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <X size={18} />
            </button>
          )}
        </div>
        ...
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.div key="s1" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} transition={{ duration: 0.2 }}>
              <StepCCCD onNext={(d, f, b) => { setCccdData(d); setCccdFront(f); setCccdBack(b); setStep(2); }} />
            </motion.div>
          )}
          {step === 2 && (
            <motion.div key="s2" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} transition={{ duration: 0.2 }}>
              <StepFace onNext={(f) => { setFaceFiles(f); setStep(3); }} onBack={() => setStep(1)} />
            </motion.div>
          )}
          {step === 3 && (
